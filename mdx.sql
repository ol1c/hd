...
WITH
  MEMBER [Measures].[Avg Duration - Current Month] AS
    ([Measures].[Average Visit Duration], [Date].[Calendar].CurrentMember)
  MEMBER [Measures].[Avg Duration - Previous Month] AS
    ([Measures].[Average Visit Duration],
...
FROM [Museum]
WHERE ([Date].[Calendar].[Month].CurrentMember)
  ...
  MEMBER [Measures].[Avg Duration Amplitude - Current Month] AS
    (
      MAX([Room].[Number].Members,
          ([Measures].[Average Visit Duration], [Date].[Calendar].CurrentMember))
      - MIN([Room].[Number].Members,
          ([Measures].[Average Visit Duration], [Date].[Calendar].CurrentMember))
    )
  MEMBER [Measures].[Avg Duration Amplitude - Previous Month] AS
    (
      MAX([Room].[Number].Members,
          ([Measures].[Average Visit Duration], ParallelPeriod([Date].[Calendar].[Month], 1, [Date].[Calendar].CurrentMember)))
      - MIN([Room].[Number].Members,
          ([Measures].[Average Visit Duration], ParallelPeriod([Date].[Calendar].[Month], 1, [Date].[Calendar].CurrentMember)))
    )
...
SELECT
  { [Measures].[Average Visit Duration] } ON COLUMNS,
  NON EMPTY
  NonEmpty(
